...
{
    public partial class EmployeeSkillsContext : DbContext
    {
        public static IConfiguration ESM_ConfigURL { get; set; }
        public EmployeeSkillsContext()
        {
        }
        public EmployeeSkillsContext(DbContextOptions<EmployeeSkillsContext> options)
            : base(options)
        {
        }
        ...
        {
            string EmployeeSkillsDBConnection = ESM_ConfigURL.GetSection("DefaultEmployeeSkillsConnection").Value;
            //MY-SQL is used to pull data and for scaffolding of DB entities in code via DB-FIRST APPROACH 
            if (!optionsBuilder.IsConfigured)
                optionsBuilder.UseMySql(EmployeeSkillsDBConnection, ServerVersion.Parse("8.0.22-mysql"));
        }
